...
export class PromptBuilderService {
  private promptsDir = path.resolve(__dirname, '../../../prompts');

  private readPromptFile(filename: string, fallback: string): string {
    try {
...
Return the output strictly matching the required JSON schema.`;
  ...
  buildUserPrompt(input: ProjectGenerationInput): string {
